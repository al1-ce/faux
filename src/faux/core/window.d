...
import faux.log;
import faux.sdl.lib;
import faux.sdl.loader;
import faux.vk.lib;
import faux.vk.context;
import faux.vk.instance;
import faux.vk.loader;
// import faux.lib.opengl.opengl;
// import faux.lib.opengl.loader;
import faux.input.event;
import faux.core.display;

private __gshared int _winCounter = 0;
private __gshared Window*[] _windows;
...
+/
struct Window {
    // import cfg = faux.config;
    // import faux.input.keyboard: Key;
    //
    ...
    /// Returns Vulkan context
    @property VkContext* context() { return &_context; }

    /// Returns Vulkan instance
    @property VkInstance* vulkanInstance() { return &_instance; }

    /// Opens new window
    bool create(int width, int height, string title) {
        if (_winCounter == 0) {
        ...
        if (_window is null) {
            ...
            _id = this.id();
            info("SDL Window ID:", _id, " created");
        }

        if (_winCounter == 0) {
            // TODO: move instance creation into 0 index
            // to allow to use that instance with multiple windows
            // but how to make it without using __gshared tho...
            // TODO: should user be able to create own vulkan instance?
            _instance = createVulkanInstance(_window);
            loadLibraryVulkanInstance(_instance);
            if (_context.isDebug) setupDebugMessenger(_instance);
        }

        VkSurfaceKHR surface;
        if (!SDL_Vulkan_CreateSurface(_window, _instance, &surface)) {
            error("Failed to create Vulkan window surface - ", fromStringz(SDL_GetError()));
        }
        _context.surface = surface;

        info("Window ID:", _id, " successfully initialised");
        _isInit = true;
        ++_winCounter;
        _windows ~= &this;
        return true;
    ...
    /// Ditto
    void create(uvec2 size, string title) {
    ...
    void close() {
        ...
        // FIXME: clean Vulkan instance
        SDL_DestroyWindow(_window);
        SDL_Quit();
        info("Successfully closed window ID:", _id);
        _isInit = false;
        _id = 0;
