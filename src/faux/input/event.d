...
/++
Module to handle input events. Please use dedicated submodules of `faux.input`
for input handling unless you need more control over it.
+/
module faux.input.event;
...
Please use methods from `faux.input.keyboard` instead
+/
...
Please use `faux.input.keyboard.lastKey()` instead
+/
...
Please use methods from `faux.input.mouse` instead
+/
